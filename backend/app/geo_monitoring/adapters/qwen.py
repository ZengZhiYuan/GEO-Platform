"""通义千问 OpenAI-compatible 官方 API 适配器。"""
...
class QwenAdapter:
    ...
    # 调用通义千问 Chat Completions API 并返回统一答案结构
    async def query(
        self,
        ...
        *,
        ...
        if not text.strip():
            raise AdapterError("qwen returned empty answer", category=ErrorCategory.INVALID_REQUEST)
...
def _require_api_key(credential: PlatformCredential) -> str:
    if not credential.api_key:
        raise AdapterError("qwen api key is required", category=ErrorCategory.UNAUTHORIZED)
