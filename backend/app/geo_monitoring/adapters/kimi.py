...
class KimiAdapter:
    ...
    async def query(
        ...
        *,
        ...
        data = response.json()
        _raise_for_content_filter(data)
        text = _extract_text(data)
        if not text.strip():
            raise AdapterError("kimi returned empty answer", category=ErrorCategory.INVALID_REQUEST)
