...
5. **环境变量** 已配置：
   ...
   - 模力指数采集（可选）：`MOLIZHISHU_ENABLED`、`MOLIZHISHU_API_TOKEN`、`MOLIZHISHU_BASE_URL`；轮询 `COLLECTION_MOLIZHISHU_MAX_POLLS`、`COLLECTION_MOLIZHISHU_POLL_DELAY_SECONDS`；回调 `MOLIZHISHU_CALLBACK_ENABLED`、`MOLIZHISHU_CALLBACK_TOKEN`；区域 `MOLIZHISHU_REGIONS_URL`
...
`collection_source` 默认 `official`。模力指数 Run 可传 `provider_mode_by_platform`、`provider_screenshot`（0/1/2）、`region_code`、`provider_callback_url`；区域列表见 `GET /providers/molizhishu/regions`。
